...
def _build_parser():
    parser = argparse.ArgumentParser(description="Simulate phenotype with main genetic and GxE effects.")
    parser.add_argument(
        "--workdir",
        default="/faststorage/project/gensubtypes/chao/code/fastgxe/example",
        help="Directory containing test.bed/.bim/.fam and where outputs are written.",
    )
    parser.add_argument("--bed-file", default="test", help="PLINK bed prefix inside workdir.")
    parser.add_argument("--out-file", default="test_simu_pheno.txt", help="Output phenotype file name.")
    parser.add_argument("--truth-file", default="test_simu_truth.txt", help="Output truth file name.")
...
remaining_after_g = np.setdiff1d(np.arange(num_snp), snp_g_indices, assume_unique=False)
snp_gxe_indices = rng.choice(remaining_after_g, size=num_snp_gxe, replace=False)
remaining_after_gxe = np.setdiff1d(remaining_after_g, snp_gxe_indices, assume_unique=False)
snp_gxe_focus_index = rng.choice(remaining_after_gxe, size=1, replace=False)
...
if total_pairs > 0:
    ...
    # scale per-pair variance so that background + focused GxE variance = h2_gxe
    h2_gxe_background = h2_gxe - h2_gxe_focus
    var_per_pair = h2_gxe_background / total_pairs
    if h2_gxe_background < 0:
    ...
    gxe_effect = _scale_to_variance(gxe_effect, h2_gxe_background)
...
gxe_focus_effect = _scale_to_variance(gxe_focus_effect, h2_gxe_focus)
...
residual = _scale_to_variance(residual, resid_var)
...
truth_rows = []
truth_rows.append({
    "effect_type": "focused_gxe_snp",
    "snp_index0": int(snp_gxe_focus_index[0]),
    "snp_id": sid_lst[snp_gxe_focus_index[0]],
    "env": ",".join([f"E{i + 1}" for i in env_focus_indices]),
    "target_variance": h2_gxe_focus,
})
for env_idx, beta in zip(env_focus_indices, snp_gxe_focus_effect):
    truth_rows.append({
        "effect_type": "focused_gxe_env",
        "snp_index0": int(snp_gxe_focus_index[0]),
        "snp_id": sid_lst[snp_gxe_focus_index[0]],
        "env": f"E{env_idx + 1}",
        "target_variance": h2_gxe_focus,
...
truth_rows.append({"effect_type": "h2_env", "target_variance": h2_env})
truth_rows.append({"effect_type": "h2_g", "target_variance": h2_g})
truth_rows.append({"effect_type": "h2_gxe_background", "target_variance": h2_gxe - h2_gxe_focus})
truth_rows.append({"effect_type": "h2_gxe_focus", "target_variance": h2_gxe_focus})
truth_rows.append({"effect_type": "resid_var", "target_variance": resid_var})
pd.DataFrame(truth_rows).to_csv(args.truth_file, sep="\t", index=False)
print("Saved simulation truth to:", args.truth_file)
