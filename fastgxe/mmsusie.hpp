...
public:
        ...
        void mmsusiefun(const MatrixXd& X, std::int64_t L, std::int64_t maxiter, double tol,
                        double coverage, double min_abs_corr, bool estimate_sigma);
        void mmsusiefun2(const MatrixXd& X, const VectorXd& y, std::int64_t L, std::int64_t maxiter, double tol,
                   double coverage, double min_abs_corr, bool estimate_sigma);

        void getCSpurity(std::vector<std::vector<int>>& cs, Eigen::VectorXd& claimed_coverage, const Eigen::MatrixXd& X, double& min_abs_corr);
        ...
        Eigen::MatrixXi in_CS(const Eigen::MatrixXd& res, double coverage = 0.9);
        Eigen::VectorXi in_CS_x(const Eigen::VectorXd& x, double coverage = 0.9);
        Eigen::VectorXd getPIP(const Eigen::MatrixXd& alpha_Mat);
